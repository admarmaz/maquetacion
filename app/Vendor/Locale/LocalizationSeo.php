...
use App\Vendor\Locale\Exceptions\SupportedLocalesNotDefined;
...
class LocalizationSeo
{
    ...
    /**
     * Class for import and export of routes.
     *
     */
    protected $manager;
     ...
     * Set rel_parent.
     *
     * @param string $rel_parent Group name
     *
     */
    ...
    /**
     * Store and return locales.
     *
     * @param array $seo values for store
     */
    public function store($seo)
    {
        ...
        foreach ($seo as $rel_anchor => $value){
            ...
            if(isset($seo['title.'.$language])){
                $seo[] = LocaleSeo::updateOrCreate([
                    'key'    => slug_helper($seo['title.'.$language]),
                    'rel_parent' => $this->rel_parent,
                    'language' => $language],[
                    'group' => 'routes',
                    'title'  => $seo['title.'.$language],
                    'subdomain' => request('domain'),
                    'url' => isset($seo['parent_page.'.$language]) ? $seo['parent_page.'.$language] . '/' . slug_helper($seo['title.'.$language]) : slug_helper($seo['title.'.$language]),
    ...
    public function getLocalesMapping()
    {
        if (empty($this->localesMapping)) {
            $this->localesMapping = $this->configRepository->get('config_seo.localesMapping');
        }
    ...
    public function getSupportedLocales()
    {
        ...
        $locales = $this->configRepository->get('config_seo.supportedLocales');
    ...
    public function getLocalesOrder()
    {
        ...
        $order = $this->configRepository->get('config_seo.localesOrder');
    ...
    {
        return $this->configRepository->get('config_seo.hideDefaultLocaleInURL');
    }
